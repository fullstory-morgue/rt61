 ...
 *      Who             When            What
 *      --------        -----------     -----------------------------
 *      JohnC           25th Aug 04     Modify from RT2500 code base
 *      JohnC           6th  Sep 04     Modified for RT2600
 *      GertjanW        21st Jan 06     Baseline code
 *      BrunoH (rt2500) 15th Feb 06     Fix for 802.11b adhoc association
 *      GertjanW        19th Feb 06     Promisc mode support
 *      MarkW (rt2500)  25th Feb 06     Fix to allow adhoc network creation
...
UCHAR Phy11BGNextRateDownward[] =
    { RATE_1, RATE_1, RATE_2, RATE_5_5, RATE_11, RATE_6, RATE_11, RATE_12,
RATE_18, RATE_24, RATE_36, RATE_48 };
UCHAR Phy11BGNextRateUpward[] =
    { RATE_2, RATE_5_5, RATE_11, RATE_12, RATE_9, RATE_12, RATE_18, RATE_24,
RATE_36, RATE_48, RATE_54, RATE_54 };

				  //  1      2       5.5      11        6        9        12      18       24       36       48       54
UCHAR Phy11ANextRateDownward[] =
    { RATE_6, RATE_6, RATE_6, RATE_6, RATE_6, RATE_6, RATE_9, RATE_12, RATE_18,
RATE_24, RATE_36, RATE_48 };
UCHAR Phy11ANextRateUpward[] =
    { RATE_9, RATE_9, RATE_9, RATE_9, RATE_9, RATE_12, RATE_18, RATE_24,
RATE_36, RATE_48, RATE_54, RATE_54 };
...
VOID AsicSwitchChannel(IN PRTMP_ADAPTER pAd, IN UCHAR Channel)
{
	...
	if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
		if (Channel <= 14) {
			if (pAd->BbpTuning.R17LowerUpperSelect == 0) {
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R17,
							     pAd->BbpTuning.
							     R17LowerBoundG);
			} else {
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R17,
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 "AsicSwitchChannel(RF=%d, Pwr=%d) to #%d, R1=0x%08x, R2=0x%08x, R3=0x%08x, R4=0x%08x\n",
...
static VOID AsicSetRxAnt(IN PRTMP_ADAPTER pAd, IN UCHAR Pair1, IN UCHAR Pair2)
{
	...
	if ((pAd->RfIcType == RFIC_2529)
	    && (pAd->Antenna.field.NumOfAntenna == 0)
	    && (pAd->Antenna.field.RxDefaultAntenna != SOFTWARE_DIVERSITY)
	    && (pAd->Antenna.field.TxDefaultAntenna != SOFTWARE_DIVERSITY)) {
		if (Pair1 != 0xFF) {
...
/*
	...
	========================================================================
*/
static VOID AsicAntennaSetting(IN PRTMP_ADAPTER pAd, IN ABGBAND_STATE BandState)
{
	...
	case RFIC_5325:
		...
		if (BandState == BG_BAND) {
			...
			if (pAd->Antenna.field.RxDefaultAntenna == ANTENNA_A) {
				...
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R77, R77);
			} else if (pAd->Antenna.field.RxDefaultAntenna ==
				   ANTENNA_B) {
				R4 = R4 | 0x01;	// <Bit5:Bit1:Bit0> = <0:0:1>
	...
	case RFIC_2529:
		...
		if (pAd->Antenna.field.NumOfAntenna == 0) {
			    ...
			    && (pAd->NicConfig2.field.TxDiversity == 1))	// Tx/Rx diversity
			{
				...
				AsicSetRxAnt(pAd, pAd->RxAnt.Pair1PrimaryRxAnt,
					     pAd->RxAnt.Pair2PrimaryRxAnt);
			} else if ((pAd->NicConfig2.field.Enable4AntDiversity == 1) && (pAd->NicConfig2.field.TxDiversity == 0))	// Tx fix & Rx diversity
			{
				...
				AsicSetRxAnt(pAd, pAd->RxAnt.Pair1PrimaryRxAnt,
					     pAd->RxAnt.Pair2PrimaryRxAnt);
			} else if ((pAd->NicConfig2.field.Enable4AntDiversity == 0) && (pAd->NicConfig2.field.TxDiversity == 1))	// Tx diversity & 2R
			{
				R4 = R4 | 0x02;	// <Bit5:Bit1:Bit0> = <0:1:0>
			...
			} else if ((pAd->NicConfig2.field.Enable4AntDiversity == 0) && (pAd->NicConfig2.field.TxDiversity == 0))	// Tx fix & 2R
			{
				if (pAd->NicConfig2.field.TxRxFixed == 0)	// TxRxFixed <Tx:Rx> = <E1/E1:E4>
				{
					R4 = R4 | 0x01;	// <Bit5:Bit1:Bit0> = <0:0:1>
...
VOID AsicAntennaSelect(IN PRTMP_ADAPTER pAd, IN UCHAR Channel)
{
	...
	DBGPRINT(RT_DEBUG_INFO, "AsicAntennaSelect(ch=%d) - Tx=%s, Rx=%s\n",
		 Channel, AntStr[pAd->Antenna.field.TxDefaultAntenna],
		 AntStr[pAd->Antenna.field.RxDefaultAntenna]);
...
/*
    ...
    ==========================================================================
 */
static VOID AsicAdjustTxPower(IN PRTMP_ADAPTER pAd)
{
	ULONG R3, CurrTxPwr;
	SHORT dbm;
	...
	if (index >= pAd->ChannelListNum) {
		DBGPRINT(RT_DEBUG_ERROR,
			 "AsicAdjustTxPower(can find pAd->PortCfg.Channel=%d in ChannelList[%d]\n",
			     ...
			     IN USHORT TbttNumToNextWakeUp)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ">>>AsicSleepThenAutoWakeup(sleep %d TU)<<<\n",
		 (csr11.field.NumOfTBTTBeforeWakeup *
		  pAd->PortCfg.BeaconPeriod) +
		 csr11.field.DelayAfterLastTBTTBeforeWakeup);
...
/*
	...
	==========================================================================
 */
static VOID AsicBbpTuning(IN PRTMP_ADAPTER pAd)
{
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
		...
		if ((dbm < -80) && (pAd->Mlme.PeriodicRound > 20)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("weak RSSI=%d, CCA=%d, stop tuning, R17 = 0x%x\n",
				  dbm, pAd->RalinkCounters.OneSecFalseCCACnt,
				  R17));
			return;
		}
		...
		if (dbm >= RSSI_FOR_VERY_LOW_SENSIBILITY) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 "strong RSSI=%d, CCA=%d, fixed R17 at 0x%x\n",
				 dbm, pAd->RalinkCounters.OneSecFalseCCACnt,
				 R17);
			return;
		}
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 "mid RSSI=%d, CCA=%d, fixed R17 at 0x%x\n",
				 dbm, pAd->RalinkCounters.OneSecFalseCCACnt,
				 R17);
			return;
		}
		...
		//          to RSSI.
			...
			if (R17 > R17UpperBound) {
				...
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, 17, R17);
				DBGPRINT(RT_DEBUG_TRACE,
					 "RSSI=%d, CCA=%d, R17=R17UpperBound=0x%x\n",
					 dbm,
	...
	if ((pAd->RalinkCounters.OneSecFalseCCACnt >
	     pAd->BbpTuning.FalseCcaUpperThreshold) && (R17 < R17UpperBound)) {
		...
		DBGPRINT(RT_DEBUG_TRACE, "RSSI=%d, CCA=%d, ++R17= 0x%x\n", dbm,
			 pAd->RalinkCounters.OneSecFalseCCACnt, R17);
	} else
	    if ((pAd->RalinkCounters.OneSecFalseCCACnt <
		 pAd->BbpTuning.FalseCcaLowerThreshold)
		&& (R17 > R17LowerBound)) {
		R17 -= pAd->BbpTuning.R17Delta;
			     ...
			     IN PUCHAR pKey, IN PUCHAR pTxMic, IN PUCHAR pRxMic)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 "AsicAddPairwiseKeyEntry: #%d Alg=%s mac=%02x:%02x:%02x:%02x:%02x:%02x key=%02x-%02x-%02x-..\n",
	...
	RTMP_IO_WRITE8(pAd, offset + ETH_ALEN, CipherAlg);
...
static VOID AsicRemovePairwiseKeyEntry(IN PRTMP_ADAPTER pAd, IN UCHAR KeyIdx)
{
	ULONG csr2, csr3;
			     ...
			     IN UCHAR Token, IN UCHAR Arg0, IN UCHAR Arg1)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 "SW interrupt MCU (cmd=0x%02x, token=0x%02x, arg1,arg0=0x%02x,0x%02x)\n",
...
static VOID AsicEvaluateSecondaryRxAnt(IN PRTMP_ADAPTER pAd)
{
	DBGPRINT(RT_DEBUG_TRACE,
		 "AntDiv - before evaluate Pair1-Ant (%d,%d), Pair2-Ant (%d,%d)\n",
		 pAd->RxAnt.Pair1PrimaryRxAnt, pAd->RxAnt.Pair1SecondaryRxAnt,
		 pAd->RxAnt.Pair2PrimaryRxAnt, pAd->RxAnt.Pair2SecondaryRxAnt);

	if (pAd->RfIcType == RFIC_2529) {
		AsicSetRxAnt(pAd, pAd->RxAnt.Pair1SecondaryRxAnt,
			     pAd->RxAnt.Pair2SecondaryRxAnt);
	} else {
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
		pAd->RxAnt.RxAntDiversityTimer.expires = jiffies + 100;
	else
		pAd->RxAnt.RxAntDiversityTimer.expires = jiffies + 300;
	add_timer(&pAd->RxAnt.RxAntDiversityTimer);
}

// this timeout routine collect AvgRssi[SecondaryRxAnt] and decide the best Ant
static VOID AsicRxAntEvalTimeout(IN unsigned long data)
{
	RTMP_ADAPTER *pAd = (RTMP_ADAPTER *) data;
	UCHAR temp;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ...
		 (pAd->RxAnt.Pair2AvgRssi[0] >> 3),
		 (pAd->RxAnt.Pair2AvgRssi[1] >> 3),
	...
	if (pAd->RfIcType == RFIC_2529) {
		if ((pAd->RxAnt.RcvPktNumWhenEvaluate != 0) && (pAd->RxAnt.Pair1AvgRssi[pAd->RxAnt.Pair1SecondaryRxAnt] >= pAd->RxAnt.Pair1AvgRssi[pAd->RxAnt.Pair1PrimaryRxAnt])) {	// select PrimaryRxAntPair
		...
		if ((pAd->RxAnt.RcvPktNumWhenEvaluate != 0) && (pAd->RxAnt.Pair2AvgRssi[pAd->RxAnt.Pair2SecondaryRxAnt] >= pAd->RxAnt.Pair2AvgRssi[pAd->RxAnt.Pair2PrimaryRxAnt])) {	// select SecondaryRxAntPair
			pAd->RxAnt.Pair2LastAvgRssi =
			    (pAd->RxAnt.
			     Pair2AvgRssi[pAd->RxAnt.Pair2SecondaryRxAnt] >> 3);
		...
		pAd->RxAnt.Pair1AvgRssi[pAd->RxAnt.Pair1SecondaryRxAnt] =
		    (-95) << 3;
		pAd->RxAnt.Pair2AvgRssi[pAd->RxAnt.Pair2SecondaryRxAnt] =
		    (-95) << 3;
	} else {
		if ((pAd->RxAnt.RcvPktNumWhenEvaluate != 0) && (pAd->RxAnt.Pair1AvgRssi[pAd->RxAnt.Pair1SecondaryRxAnt] >= pAd->RxAnt.Pair1AvgRssi[pAd->RxAnt.Pair1PrimaryRxAnt])) {	// select PrimaryRxAntPair
			temp = pAd->RxAnt.Pair1PrimaryRxAnt;
...
static VOID MlmeAutoScan(IN PRTMP_ADAPTER pAd)
{
...
static VOID MlmeAutoRecoverNetwork(IN PRTMP_ADAPTER pAd)
{
	// check CntlMachine.CurrState to avoid collision with NDIS SetOID request
	if (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE) {
...
static VOID MlmeCheckForRoaming(IN PRTMP_ADAPTER pAd, IN ULONG Now32)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 "==> MlmeCheckForRoaming::pAd->ScanTab.BssNr = %d\n",
		 pAd->ScanTab.BssNr);
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 "MlmeCheckForRoaming::pBss->LastBeaconRxTime = %ld\n",
			 pBss->LastBeaconRxTime);

		if ((pBss->LastBeaconRxTime + BEACON_LOST_TIME) < Now32) {
			DBGPRINT(RT_DEBUG_TRACE,
				 "1: AP disappear::Now32 = %d\n", Now32);
			continue;	// AP disappear
		}
		if (pBss->Rssi <= RSSI_THRESHOLD_FOR_ROAMING) {
			DBGPRINT(RT_DEBUG_TRACE,
				 "2: RSSI too weak::Rssi[%d] - RSSI_THRESHOLD_FOR_ROAMING[%d]\n",
				 pBss->Rssi, RSSI_THRESHOLD_FOR_ROAMING);
			continue;	// RSSI too weak. forget it.
		}
		if (memcmp(pBss->Bssid, pAd->PortCfg.Bssid, ETH_ALEN) == 0) {
			DBGPRINT(RT_DEBUG_TRACE, "3: skip current AP\n");
			continue;	// skip current AP
		}
		if (!SSID_EQUAL
		    (pBss->Ssid, pBss->SsidLen, pAd->PortCfg.Ssid,
		     pAd->PortCfg.SsidLen)) {
		...
		if (pBss->Rssi < (pAd->PortCfg.LastRssi + RSSI_DELTA)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 "5: only AP with stronger RSSI is eligible for roaming\n");
...
/*
	...
	==========================================================================
 */
static VOID MlmeCheckForFastRoaming(IN PRTMP_ADAPTER pAd, IN unsigned long Now)
{
	USHORT i;
	BSS_TABLE *pRoamTab = &pAd->MlmeAux.RoamTab;
	BSS_ENTRY *pBss;
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		if ((pBss->Rssi <= 45)
		    && (pBss->Channel == pAd->PortCfg.Channel))
			continue;	// RSSI too weak. forget it.
		if (memcmp(pBss->Bssid, pAd->PortCfg.Bssid, ETH_ALEN) == 0)
			continue;	// skip current AP
		if (!SSID_EQUAL
...
/*
    ...
    ==========================================================================
 */
static VOID MlmeCalculateChannelQuality(IN PRTMP_ADAPTER pAd, IN ULONG Now32)
{
	ULONG TxOkCnt, TxCnt;
	ULONG RxCnt;
	...
	if (TxCnt > 5) {
		...
		TxPRR =
		    ((TxCnt -
		      pAd->RalinkCounters.OneSecTxNoRetryOkCount) * 100) /
		    TxCnt;
	}
	//
	...
	//
	if (INFRA_ON(pAd) && (TxOkCnt < 2) &&	// no heavy traffic
	    (pAd->PortCfg.LastBeaconRxTime + BEACON_LOST_TIME < Now32)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 "BEACON lost > %d msec with TxOkCnt=%d -> CQI=0\n",
	...
	DBGPRINT(RT_DEBUG_INFO,
		 ...
		 pAd->RalinkCounters.OneSecTxRetryOkCount, TxCnt,
		 pAd->RalinkCounters.OneSecRxFcsErrCnt, RxCnt,
...
/*
    ...
    ==========================================================================
 */
static VOID MlmeDynamicTxRateSwitching(IN PRTMP_ADAPTER pAd)
{
	...
	NewBasicRateBitmap =
	    pAd->PortCfg.BasicRateBitmap & BasicRateMask[CurrRate];
	...
	if (TxTotalCnt)
		TxErrorRatio =
		    ((pAd->RalinkCounters.OneSecTxRetryOkCount +
		      pAd->RalinkCounters.OneSecTxFailCount) * 100) /
		    TxTotalCnt;

	DBGPRINT_RAW(RT_DEBUG_TRACE,
		     "%d: NDIS push BE=%d, BK=%d, VI=%d, VO=%d, TX/RX AGGR=<%d,%d>, p-NDIS=%d, RSSI=%d, ACKbmap=%03x, PER=%d%%\n",
		     ...
		     pAd->RalinkCounters.PendingNdisPacketCount, dbm,
		     NewBasicRateBitmap & 0xfff, TxErrorRatio);
	...
	if (TxTotalCnt <= 15) {
		...
		memset(pAd->DrsCounters.TxQuality, 0,
	...
	do {
		...
		if (TxErrorRatio >= RateDownPER[CurrRate]) {
			pAd->DrsCounters.TxQuality[CurrRate] =
		...
		if ((UpRate != CurrRate) &&
		    ...
		    ((pAd->DrsCounters.PER[CurrRate] > 20)
		     || (pAd->DrsCounters.fNoisyEnvironment))
		    && ((pAd->DrsCounters.PER[CurrRate] + 5) >
			pAd->DrsCounters.PER[UpRate])) {
			...
			UpRate++;
			if ((UpRate == RATE_6) || (UpRate == RATE_9))
				UpRate = RATE_12;
			if (UpRate > pAd->PortCfg.MaxTxRate)
				UpRate = pAd->PortCfg.MaxTxRate;
		...
		if ((pAd->DrsCounters.fNoisyEnvironment == TRUE) &&
			...
			for (JumpUpRate = RATE_54; JumpUpRate > RATE_1;
			     JumpUpRate--) {
		...
		if ((pAd->DrsCounters.TxQuality[CurrRate] >=
		     DRS_TX_QUALITY_WORST_BOUND) && (CurrRate != DownRate)) {
#if 1
	...
	if (pAd->PortCfg.TxRate > CurrRate) {
		DBGPRINT(RT_DEBUG_TRACE, "DRS: ++TX rate from %d to %d Mbps\n",
			 RateIdToMbps[CurrRate],
			 RateIdToMbps[pAd->PortCfg.TxRate]);
		...
		if (!pAd->PortCfg.QuickResponeForRateUpTimerRunning) {
			if (pAd->PortCfg.TxRate <= RATE_12)
				...
				pAd->PortCfg.QuickResponeForRateUpTimer.
				    expires = jiffies + 100;
			add_timer(&pAd->PortCfg.QuickResponeForRateUpTimer);
...
/*
    ...
    ==========================================================================
 */
static VOID MlmeCheckPsmChange(IN PRTMP_ADAPTER pAd, IN ULONG Now32)
{
	ULONG PowerMode;
	// condition -
...
static VOID STAMlmePeriodicExec(PRTMP_ADAPTER pAd)
{
	...
	if (pAd->PortCfg.bBlockAssoc
	    && (pAd->PortCfg.LastMicErrorTime + (60 * HZ) < pAd->Mlme.Now32))
		pAd->PortCfg.bBlockAssoc = FALSE;

	DBGPRINT(RT_DEBUG_INFO,
		 "MMCHK - PortCfg.Ssid[%d]=%c%c%c%c... MlmeAux.Ssid[%d]=%c%c%c%c...\n",
		 pAd->PortCfg.SsidLen, pAd->PortCfg.Ssid[0],
		 pAd->PortCfg.Ssid[1], pAd->PortCfg.Ssid[2],
		 pAd->PortCfg.Ssid[3], pAd->MlmeAux.SsidLen,
	...
	if ((((pAd->RfIcType == RFIC_2529)
		...
		if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
			...
			if (pAd->PortCfg.NumOfAvgRssiSample < 5) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 "MlmePeriodicExec: no traffic/beacon, reset RSSI\n");
			} else
			...
			DBGPRINT(RT_DEBUG_INFO,
				 "Ant-realrssi0(%d),Lastrssi0(%d)\n",
				 realavgrssi1, pAd->RxAnt.Pair1LastAvgRssi);

			if ((realavgrssi1 > (pAd->RxAnt.Pair1LastAvgRssi + 5))
			    || (realavgrssi1 <
				(pAd->RxAnt.Pair1LastAvgRssi - 5))) {
				evaluate = TRUE;
			...
			if (pAd->RfIcType == RFIC_2529) {
				realavgrssi2 =
				    (pAd->RxAnt.
				     Pair2AvgRssi[pAd->RxAnt.
						  Pair2PrimaryRxAnt] >> 3);

				DBGPRINT(RT_DEBUG_INFO,
					 "Ant-realrssi1(%d),Lastrssi1(%d)\n",
					 realavgrssi2,
					 pAd->RxAnt.Pair2LastAvgRssi);

				if ((realavgrssi1 >
				     (pAd->RxAnt.Pair2LastAvgRssi + 5))
				    || (realavgrssi2 <
					(pAd->RxAnt.Pair2LastAvgRssi - 5))) {
					evaluate = TRUE;
					pAd->RxAnt.Pair2LastAvgRssi =
	...
	if ((pAd->Mlme.SyncMachine.CurrState == SYNC_IDLE)
	    && (pAd->PortCfg.Channel <= 14)) {
		if (max(pAd->PortCfg.LastRssi, pAd->PortCfg.LastRssi2) >=
		    (-82 + pAd->BbpRssiToDbmDelta)) {
	...
	if (INFRA_ON(pAd)) {
		...
		if (INFRA_ON(pAd) && (pAd->PortCfg.LastBeaconRxTime + 2500 < pAd->Mlme.Now32))	// BEACON almost starving?
		{
			DBGPRINT(RT_DEBUG_TRACE,
				 "!!! BEACON lost > 2.5 sec !!! Send ProbeRequest\n");
		...
		//
		if (INFRA_ON(pAd) &&
		    (pAd->PortCfg.LastBeaconRxTime + 1000 < pAd->Mlme.Now32) &&
		...
		if ((pAd->PortCfg.Channel <= 14) &&
		    ...
		    ((pAd->PortCfg.Last11bBeaconRxTime + 5000) <
		     pAd->Mlme.Now32)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 "MMCHK - last 11B peer left, update Tx rates\n");

			memcpy(pAd->ActiveCfg.SupRate, pAd->PortCfg.SupRate,
			       MAX_LEN_OF_SUPPORTED_RATES);
			...
			if (pAd->PortCfg.RadarDetect.RDMode == RD_SILENCE_MODE) {
				...
				if (pAd->PortCfg.RadarDetect.RDCount > 65) {
					if (RadarDetectionStop(pAd)) {
						...
						DBGPRINT(RT_DEBUG_TRACE,
							 "Not found radar signal, start send beacon\n");
						AsicEnableIbssSync(pAd);
						pAd->PortCfg.RadarDetect.
						    RDMode = RD_NORMAL_MODE;
					}
				}
			}
		}
		...
		// join later.
		if ((pAd->PortCfg.LastBeaconRxTime + ADHOC_BEACON_LOST_TIME <
		     pAd->Mlme.Now32)
	...
	} else			// no INFRA nor ADHOC connection
	{
		...
		if ((pAd->PortCfg.bAutoReconnect == TRUE)
			...
			if ((pAd->ScanTab.BssNr == 0)
				...
				if ((pAd->PortCfg.LastScanTime + 10 * HZ) <
				    pAd->Mlme.Now32) {
					DBGPRINT(RT_DEBUG_TRACE,
						 "CNTL - No matching BSS, start a new ACTIVE scan SSID[%s]\n",
						 pAd->MlmeAux.
						 AutoReconnectSsid);
					ScanParmFill(pAd, &ScanReq,
						     pAd->MlmeAux.
				...
				if ((pAd->Mlme.PeriodicRound % 10) == 7) {
					if ((pAd->PortCfg.LastScanTime +
					     10 * HZ) < pAd->Mlme.Now32) {
						MlmeAutoScan(pAd);
						pAd->PortCfg.LastScanTime =
						    pAd->Mlme.Now32;
					}
...
VOID MlmeAutoReconnectLastSSID(IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE &&
		...
		memcpy(OidSsid.Ssid, pAd->MlmeAux.AutoReconnectSsid,
...
VOID MlmeUpdateTxRates(IN PRTMP_ADAPTER pAd, IN BOOLEAN bLinkUp)
{
	...
	if ((pAd->PortCfg.BssType == BSS_ADHOC) &&
	    ...
	    (pAd->PortCfg.AdhocMode == 0) && (MaxDesire > RATE_11))
		MaxDesire = RATE_11;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ...
		 RateIdToMbps[pAd->PortCfg.MaxTxRate],
		 RateIdToMbps[pAd->PortCfg.MinTxRate], OPSTATUS_TEST_FLAG(pAd,
									  fOP_STATUS_TX_RATE_SWITCH_ENABLED));
	DBGPRINT(RT_DEBUG_TRACE,
		 " MlmeUpdateTxRates (TxRate=%d, RtsRate=%d, BasicRateBitmap=0x%04x)\n",
		 RateIdToMbps[pAd->PortCfg.TxRate],
		 RateIdToMbps[pAd->PortCfg.RtsRate], BasicRateBitmap);
}
...
static VOID BssCipherParse(IN OUT PBSS_ENTRY pBss)
{
	PEID_STRUCT pEid;
	PUCHAR pTmp;
	...
	while (Length > 0) {
		...
		case IE_RSN:
			...
			pBss->WPA2.RsnCapability =
			    SWAP16(pBss->WPA2.RsnCapability);
#endif
			pTmp += sizeof(USHORT);
...
static ULONG BssTableSearchWithSSID(IN BSS_TABLE * Tab,
			     IN PUCHAR Bssid,
			     IN PUCHAR pSsid,
			     IN UCHAR SsidLen, IN UCHAR Channel)
{
	...
	for (i = 0; i < Tab->BssNr; i++) {
		if ((((Tab->BssEntry[i].Channel <= 14) && (Channel <= 14)) ||
		     ((Tab->BssEntry[i].Channel > 14) && (Channel > 14))) &&
		    (memcmp(&(Tab->BssEntry[i].Bssid), Bssid, ETH_ALEN) == 0) &&
		    (SSID_EQUAL
 ...
 *  \post
 */
static VOID BssEntrySet(IN PRTMP_ADAPTER pAd,
		 OUT BSS_ENTRY * pBss,
		 IN PUCHAR pBssid,
		 IN CHAR Ssid[],
		 IN UCHAR SsidLen,
		 ...
		 IN UCHAR LengthVIE, IN PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	memcpy(pBss->Bssid, pBssid, ETH_ALEN);
	...
	if (pQosCapability)
		memcpy(&pBss->QosCapability, pQosCapability,
	...
	if (Idx == BSS_NOT_FOUND) {
		if (Tab->BssNr >= MAX_LEN_OF_BSS_TABLE) {
			...
			if (!OPSTATUS_TEST_FLAG
				...
				if (memcmp(pAd->MlmeAux.Bssid, pBssid, ETH_ALEN)
				    == 0
		...
		BssEntrySet(pAd, &Tab->BssEntry[Idx], pBssid, Ssid, SsidLen,
			    ...
			    ExtRateLen, ChannelNo, Rssi, TimeStamp, CkipFlag,
			    pEdcaParm, pQosCapability, pQbssLoad, LengthVIE,
			    pVIE);
		Tab->BssNr++;
	} else {
		BssEntrySet(pAd, &Tab->BssEntry[Idx], pBssid, Ssid, SsidLen,
		      ...
		      OUT BSS_TABLE * OutTab, IN CHAR Ssid[], IN UCHAR SsidLen)
{
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		if ((pInBss->BssType == pAd->PortCfg.BssType) &&
			...
			// We will check wepstatus for qualification Bss
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 "PortCfg.WepStatus=%d, while pInBss->WepStatus=%d\n",
					 pAd->PortCfg.WepStatus,
					 pInBss->WepStatus);
				continue;
			}
			...
			OutTab->BssNr++;
		} else if ((pInBss->BssType == pAd->PortCfg.BssType)
			   && (SsidLen == 0)) {
			BSS_ENTRY *pOutBss = &OutTab->BssEntry[OutTab->BssNr];
	...
	if (OutTab->BssNr > 0) {
		if (pAd->PortCfg.AuthMode == Ndis802_11AuthModeWPA2PSK)
			RTMPMakeRSNIE(pAd,
				      OutTab->BssEntry[0].WPA2.GroupCipher);
	...
	for (i = 0; i < OutTab->BssNr - 1; i++) {
		for (j = i + 1; j < OutTab->BssNr; j++) {
			if (OutTab->BssEntry[j].Rssi > OutTab->BssEntry[i].Rssi) {
				memcpy(&TmpBss, &OutTab->BssEntry[j],
				       sizeof(BSS_ENTRY));
				memcpy(&OutTab->BssEntry[j],
				       &OutTab->BssEntry[i], sizeof(BSS_ENTRY));
				memcpy(&OutTab->BssEntry[i], &TmpBss,
				       sizeof(BSS_ENTRY));
			}
		}
	}
		    ...
		    IN ULONG MsgType, IN ULONG MsgLen, IN VOID * Msg)
{
	...
	if (MsgLen > MAX_LEN_OF_MLME_BUFFER) {
		DBGPRINT(RT_DEBUG_ERROR,
			 "MlmeEnqueue: msg too large, size = %d \n", MsgLen);
		return FALSE;
	}
			   ...
			   IN ULONG MsgLen, IN VOID * Msg, IN UCHAR Signal)
{
	...
	if (MsgLen > MAX_LEN_OF_MLME_BUFFER) {
		DBGPRINT(RT_DEBUG_ERROR,
			 "MlmeEnqueueForRecv: frame too large, size = %d \n",
			 MsgLen);
		return FALSE;
	}
	...
	if ((Machine == SYNC_STATE_MACHINE) && (MsgType == MT2_PEER_PROBE_RSP)
	    && (pAd->PortCfg.bGetAPConfig)) {
		if (pAd->Mlme.SyncMachine.CurrState == SYNC_IDLE) {
...
VOID MlmeRestartStateMachine(IN PRTMP_ADAPTER pAd)
{
	...
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_MLME_INITIALIZED)) {
		...
		if (MlmeInit(pAd))
			DBGPRINT(RT_DEBUG_ERROR,
				 "Failure to initialize mlme.\n");
		// Continue the reset procedure...
	}
#if SL_IRQSAVE
	spin_lock_irqsave(&pAd->Mlme.Queue.Lock, IrqFlags);
#else
	spin_lock_bh(&pAd->Mlme.Queue.Lock);
#endif

	if (pAd->Mlme.Running) {
#if SL_IRQSAVE
		spin_unlock_irqrestore(&pAd->Mlme.Queue.Lock,
				       IrqFlags);
#else
		spin_unlock_bh(&pAd->Mlme.Queue.Lock);
#endif
		return;
	} else {
...
#if SL_IRQSAVE
	spin_unlock_irqrestore(&pAd->Mlme.Queue.Lock, IrqFlags);
#else
	spin_unlock_bh(&pAd->Mlme.Queue.Lock);
#endif
...
#if SL_IRQSAVE
	spin_lock_irqsave(&(pQueue->Lock), IrqFlags);
#else
	...
	if (pFrame->Hdr.FC.Type == BTYPE_DATA) {
		*Machine = WPA_PSK_STATE_MACHINE;
		...
		if ((pAd->PortCfg.bWmmCapable)
		    && (pFrame->Hdr.FC.SubType == SUBTYPE_QDATA)) {
			EAPType = *((UCHAR *) pFrame + LENGTH_802_11 + LENGTH_802_1_H + LENGTH_WMMQOS_H + 1);	//add QoS frames
		} else {
			EAPType =
 ...
 *  \return   None
 */
static VOID StateMachinePerformAction(IN PRTMP_ADAPTER pAd,
...
NDIS_STATUS MlmeInit(IN PRTMP_ADAPTER pAd)
{
	...
	do {
		pAd->Mlme.Running = FALSE;
		spin_lock_init(&pAd->Mlme.Queue.Lock);
		spin_lock_init(&pAd->Mlme.MemLock);
		...
		ASSERT(AUTH_RSP_FUNC_SIZE ==
		       MAX_AUTH_RSP_MSG * MAX_AUTH_RSP_STATE);
		...
		ASSERT(WPA_PSK_FUNC_SIZE ==
		       MAX_WPA_PSK_MSG * MAX_WPA_PSK_STATE);
		...
		init_timer(&pAd->Mlme.LinkDownTimer);
		...
		pAd->RxAnt.RxAntDiversityTimer.function = &AsicRxAntEvalTimeout;
...
VOID MlmeHalt(IN PRTMP_ADAPTER pAd)
{
	...
	if (INFRA_ON(pAd) && !RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)) {
		memcpy(DisReq.Addr, pAd->PortCfg.Bssid, ETH_ALEN);
...
VOID StaQuickResponeForRateUpExec(IN unsigned long data)
{
	...
	do {
		...
		if ((pAd->PortCfg.Channel > 14) ||	// must be in 802.11A band
		    (pAd->PortCfg.PhyMode == PHY_11G))	// G-only mode, no CCK rates available
		{
			if (Phy11ANextRateUpward[CurrRate] <=
			...
			if (pAd->PortCfg.MaxTxRate < RATE_FIRST_OFDM_RATE) {
				if (Phy11BNextRateUpward[CurrRate] <=
				    pAd->PortCfg.MaxTxRate)
				...
				if (Phy11BGNextRateUpward[CurrRate] <=
				    pAd->PortCfg.MaxTxRate)
					UpRate =
					    Phy11BGNextRateUpward[CurrRate];
				else
					UpRate = CurrRate;
				DownRate = Phy11BGNextRateDownward[CurrRate];
			}
		...
		if (pAd->PortCfg.TxRate < CurrRate) {
			DBGPRINT(RT_DEBUG_TRACE,
				 "StaQuickResponeForRateUpExec: Before TX rate = %d Mbps, Now Tx rate = %d Mbps\n",
				 RateIdToMbps[CurrRate],
				 RateIdToMbps[pAd->PortCfg.TxRate]);
...
VOID RadarDetectionStart(IN PRTMP_ADAPTER pAd)
{
	...
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R18,
				    &pAd->PortCfg.RadarDetect.BBPR18);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R18, 0xFF);

	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R21,
				    &pAd->PortCfg.RadarDetect.BBPR21);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R21, 0x3F);
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R22,
				    &pAd->PortCfg.RadarDetect.BBPR22);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R22, 0x3F);

	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R16,
				    &pAd->PortCfg.RadarDetect.BBPR16);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R16, 0xBD);
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R17,
				    &pAd->PortCfg.RadarDetect.BBPR17);
	if (pAd->NicConfig2.field.ExternalLNAForA) {
	...
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R64,
				    &pAd->PortCfg.RadarDetect.BBPR64);
...
BOOLEAN RadarDetectionStop(IN PRTMP_ADAPTER pAd)
{
	...
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R16,
				     pAd->PortCfg.RadarDetect.BBPR16);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R17,
				     pAd->PortCfg.RadarDetect.BBPR17);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R18,
				     pAd->PortCfg.RadarDetect.BBPR18);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R21,
				     pAd->PortCfg.RadarDetect.BBPR21);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R22,
				     pAd->PortCfg.RadarDetect.BBPR22);
