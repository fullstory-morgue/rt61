...
{
	StateMachineInit(S, (STATE_MACHINE_FUNC *) Trans, MAX_ASSOC_STATE,
			 MAX_ASSOC_MSG, (STATE_MACHINE_FUNC) Drop, ASSOC_IDLE,
			 ASSOC_MACHINE_BASE);
	...
	// timer init
	init_timer(&pAd->MlmeAux.AssocTimer);
	pAd->MlmeAux.AssocTimer.data = (unsigned long)pAd;
	pAd->MlmeAux.AssocTimer.function = &AssocTimeout;

	init_timer(&pAd->MlmeAux.ReassocTimer);
	pAd->MlmeAux.ReassocTimer.data = (unsigned long)pAd;
	pAd->MlmeAux.ReassocTimer.function = &ReassocTimeout;

	init_timer(&pAd->MlmeAux.DisassocTimer);
	pAd->MlmeAux.DisassocTimer.data = (unsigned long)pAd;
	pAd->MlmeAux.DisassocTimer.function = &DisassocTimeout;
}
...
VOID MlmeAssocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	// check sanity first
		...
		pAd->PortCfg.AssocInfo.RequestFixedIEs.Capabilities =
		    CapabilityInfo;
		pAd->PortCfg.AssocInfo.RequestFixedIEs.ListenInterval =
		    ListenIntv;
		// Only reassociate need this
		//memcpy(pAd->PortCfg.AssocInfo.RequestFixedIEs.CurrentAPAddress, ApAddr, ETH_ALEN);
		pAd->PortCfg.AssocInfo.OffsetRequestIEs =
		    sizeof(NDIS_802_11_ASSOCIATION_INFORMATION);
		...
		VarIesOffset += 1;
		memcpy(pAd->PortCfg.ReqVarIEs + VarIesOffset,
		       &pAd->MlmeAux.SsidLen, 1);
		VarIesOffset += 1;
		memcpy(pAd->PortCfg.ReqVarIEs + VarIesOffset, pAd->MlmeAux.Ssid,
		       pAd->MlmeAux.SsidLen);
		...
		if ((pAd->PortCfg.AuthMode == Ndis802_11AuthModeWPA) ||
			...
			// Append Variable IE
			memcpy(pAd->PortCfg.ReqVarIEs + VarIesOffset, CipherTmp,
			       CipherTmpLen);
			...
			pAd->PortCfg.AssocInfo.OffsetResponseIEs =
			    sizeof(NDIS_802_11_ASSOCIATION_INFORMATION) +
			    pAd->PortCfg.ReqVarIELen;
		}
		// For WPA2 / WPA2-PSK
			...
			if (pAd->PortCfg.AuthMode == Ndis802_11AuthModeWPA2) {
				...
				for (idx = 0; idx < PMKID_NO; idx++) {
					if (memcmp
					    (ApAddr,
					     &pAd->PortCfg.SavedPMK[idx].BSSID,
					     6) == 0) {
						FoundPMK = TRUE;
						break;
					}
				...
				if (FoundPMK) {
					...
					CipherTmp[1] += 18;
					// Set PMK number
					*(PUSHORT) & CipherTmp[CipherTmpLen] =
					    1;
					memcpy(&CipherTmp[CipherTmpLen + 2],
					       &pAd->PortCfg.SavedPMK[idx].
					       PMKID, 16);
					CipherTmpLen += 18;
				}
			}
		...
		pAd->MlmeAux.AssocTimer.expires = jiffies + Timeout;	/* in mSec */
		add_timer(&pAd->MlmeAux.AssocTimer);
...
VOID MlmeReassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	// the parameters are the same as the association
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11),
				  ...
				  &ListenIntv, ETH_ALEN, ApAddr, 1, &SsidIe, 1,
		...
		pAd->MlmeAux.ReassocTimer.expires = jiffies + Timeout;	/* in mSec */
		add_timer(&pAd->MlmeAux.ReassocTimer);
...
//#endif
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 "ASSOC - Send DISASSOC request [BSSID::%02x:%02x:%02x:%02x:%02x:%02x] \n",
	...
	MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0,
			 pDisassocReq->Addr, pDisassocReq->Addr);
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11),
...
//#endif
	...
	pAd->MlmeAux.DisassocTimer.expires = jiffies + Timeout;	/* in mSec */
	add_timer(&pAd->MlmeAux.DisassocTimer);
...
NDIS_STATUS miniport_query_info(IN PRTMP_ADAPTER pAd,
				IN USHORT oid, OUT void *buf, OUT ULONG bufsize)
{
	// First add AssocInfo
	memcpy(buf, &pAd->PortCfg.AssocInfo,
	       sizeof(NDIS_802_11_ASSOCIATION_INFORMATION));
	// Second add ReqVarIEs
	memcpy(buf + sizeof(NDIS_802_11_ASSOCIATION_INFORMATION),
	       pAd->PortCfg.ReqVarIEs, pAd->PortCfg.ReqVarIELen);
	// Third add ResVarIEs
	memcpy(buf + sizeof(NDIS_802_11_ASSOCIATION_INFORMATION) +
	       pAd->PortCfg.ReqVarIELen, pAd->PortCfg.ResVarIEs,
	       pAd->PortCfg.ResVarIELen);

	return 0;
}

VOID link_status_handler(IN PRTMP_ADAPTER pAd)
{
...
//#endif
	...
	if (PeerAssocRspSanity
	    (pAd, Elem->Msg, Elem->MsgLen, Addr2, &CapabilityInfo, &Status,
	     &Aid, SupRate, &SupRateLen, ExtRate, &ExtRateLen, &EdcaParm)) {
		// The frame is for me ?
		if (memcmp(Addr2, pAd->MlmeAux.Bssid, ETH_ALEN) == 0) {
			DBGPRINT(RT_DEBUG_TRACE,
...
//#endif
	...
	if (PeerAssocRspSanity
	    (pAd, Elem->Msg, Elem->MsgLen, Addr2, &CapabilityInfo, &Status,
	     &Aid, SupRate, &SupRateLen, ExtRate, &ExtRateLen, &EdcaParm)) {
		if (memcmp(Addr2, pAd->MlmeAux.Bssid, ETH_ALEN) == 0)	// The frame is for me ?
		{
			DBGPRINT(RT_DEBUG_TRACE,
		   ...
		   IN UCHAR ExtRateLen, IN PEDCA_PARM pEdcaParm)
{
	...
	if (Idx == BSS_NOT_FOUND) {
		...
		pAd->PortCfg.AssocInfo.ResponseFixedIEs.Capabilities =
		    CapabilityInfo;
		...
		VarIesOffset += 1;
		memcpy(pAd->PortCfg.ResVarIEs + VarIesOffset, SupRate,
		       SupRateLen);
...
//              VarIesOffset += ExtRateLen;
		...
		// Second add RSN
		memcpy(pAd->PortCfg.ResVarIEs + VarIesOffset,
		       pAd->ScanTab.BssEntry[Idx].VarIEs,
		       pAd->ScanTab.BssEntry[Idx].VarIELen);
...
//#endif
	...
	if (PeerDisassocSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Reason)) {
		if (INFRA_ON(pAd)
		    && (memcmp(pAd->PortCfg.Bssid, Addr2, ETH_ALEN) == 0)) {
			LinkDown(pAd, TRUE);
			pAd->Mlme.AssocMachine.CurrState = ASSOC_IDLE;
...
VOID Cls3errAction(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr)
{
	...
	memcpy(pAd->PortCfg.DisassocSta, pAddr, ETH_ALEN);
}
