		       ...
		       IN PUCHAR pSA, IN UCHAR UserPriority, IN PUCHAR pMICKey)
{
	...
	// DA
	RTMPTkipAppend(&pAdapter->PrivateInfo.Tx, pDA, ETH_ALEN);
	// SA
	RTMPTkipAppend(&pAdapter->PrivateInfo.Tx, pSA, ETH_ALEN);
				...
				IN PUCHAR pSA, IN PUCHAR pMICKey, IN UINT Len)
{
	...
	// DA
	RTMPTkipAppend(&pAdapter->PrivateInfo.Rx, pDA, ETH_ALEN);
	// SA
	RTMPTkipAppend(&pAdapter->PrivateInfo.Rx, pSA, ETH_ALEN);
	// Priority + 3 bytes of 0
			   ...
			   IN PUCHAR pEncap, IN PCIPHER_KEY pKey)
{
	...
	RTMPInitMICEngine(pAdapter,
			  pKey->Key, pSrc, pSrc + 6, UserPriority, pKey->TxMic);
