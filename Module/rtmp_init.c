...
NDIS_STATUS NICLoadFirmware(IN PRTMP_ADAPTER pAd)
{
	...
	if (pFirmwareImage == NULL) {
		DBGPRINT(RT_DEBUG_ERROR,
			 "NICLoadFirmware-Memory allocate fail\n");
	...
	if (src && *src) {
		...
		if (IS_ERR(srcf)) {
			...
			if (srcf->f_op && srcf->f_op->read) {
				...
				FileLength =
				    srcf->f_op->read(srcf, pFirmwareImage,
						     MAX_FIRMWARE_IMAGE_SIZE,
						     &srcf->f_pos);
				if (FileLength != MAX_FIRMWARE_IMAGE_SIZE) {
					DBGPRINT(RT_DEBUG_ERROR,
						 ...
						 FileLength);
					Status = NDIS_STATUS_FAILURE;
				} else {	//FileLength == MAX_FIRMWARE_IMAGE_SIZE
					PUCHAR ptr = pFirmwareImage;
					USHORT crc = 0;

					for (i = 0;
					     i < (MAX_FIRMWARE_IMAGE_SIZE - 2);
					     i++, ptr++)
						...
						if ((pAd->FirmwareVersion) >
						    ((pFirmwareImage
						     ...
						     [MAX_FIRMWARE_IMAGE_SIZE -
						      3])) {
							DBGPRINT(RT_DEBUG_ERROR,
								 "NICLoadFirmware: Ver=%d.%d, local Ver=%d.%d, used FirmwareImage talbe instead\n",
								 pFirmwareImage
								 [MAX_FIRMWARE_IMAGE_SIZE
								  - 4],
								 pFirmwareImage
		...
		if (IS_ERR(srcf)) {
			...
			if (srcf->f_op && srcf->f_op->read) {
				...
				if (retval < 0) {
					...
					//NetworkType
					if (RTMPGetKeyParameter
					    ("NetworkType", tmpbuf, 25,
					     buffer)) {
						pAd->bConfigChanged = TRUE;
						if (strcmp(tmpbuf, "Adhoc") ==
						    0)
							pAd->PortCfg.BssType =
							    BSS_ADHOC;
					...
					//TxRate
					if (RTMPGetKeyParameter
					    ("TxRate", tmpbuf, 10, buffer)) {
						ulInfo =
						    (ULONG)
						    simple_strtol(tmpbuf, 0,
								  10);

						if ((pAd->PortCfg.PhyMode ==
						     PHY_11B && ulInfo <= 4)
						    ||
						    ((pAd->PortCfg.PhyMode ==
						      PHY_11BG_MIXED
					...
					//Channel
					if (RTMPGetKeyParameter
					    ("Channel", tmpbuf, 10, buffer)) {
						Channel =
						    (UCHAR)
						    simple_strtol(tmpbuf, 0,
								  10);
						pAd->PortCfg.AdHocChannel =
						    Channel;
					...
					//BGProtection
					if (RTMPGetKeyParameter
					    ("BGProtection", tmpbuf, 10,
					     buffer)) {
						switch (simple_strtol
							(tmpbuf, 0, 10)) {
						case 1:	//Always On
							pAd->PortCfg.
							    UseBGProtection = 1;
					...
					//POWER_MODE
					if (RTMPGetKeyParameter
					    ("PSMode", tmpbuf, 10, buffer)) {
						if (pAd->PortCfg.BssType ==
						    BSS_INFRA) {
							if ((strcmp
							     (tmpbuf,
							      "MAX_PSP") == 0)
								...
								DBGPRINT
								    (RT_DEBUG_INFO,
								     "MAX_PSP power mode not available - defaulting to CAM\n");
							} else
							    if ((strcmp
								 (tmpbuf,
								  "Fast_PSP") ==
								...
								DBGPRINT
								    (RT_DEBUG_INFO,
								     "FAST_PSP power mode not available - defaulting to CAM\n");
							}
					...
					//AuthMode
					if (RTMPGetKeyParameter
					    ("AuthMode", tmpbuf, 32, buffer)) {
						if ((strcmp(tmpbuf, "WEPAUTO")
						     == 0)
						    ||
						    (strcmp(tmpbuf, "wepauto")
						     == 0))
