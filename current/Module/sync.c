 ...
 *      Who             When            What
 *      --------        -----------     -----------------------------
 *      JohnC		1st  Sep 04     Modified for rt2561/2661
...
static VOID MlmeJoinReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	pAd->MlmeAux.BeaconTimer.expires = jiffies + JOIN_TIMEOUT;
	add_timer(&pAd->MlmeAux.BeaconTimer);
...
static VOID MlmeStartReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (MlmeStartReqSanity(pAd, Elem->Msg, Elem->MsgLen, Ssid, &SsidLen)) {
		...
		Privacy =
		    (pAd->PortCfg.WepStatus == Ndis802_11Encryption1Enabled)
		...
		memcpy(pAd->MlmeAux.SupRate, pAd->PortCfg.SupRate,
		       MAX_LEN_OF_SUPPORTED_RATES);
		RTMPCheckRates(pAd, pAd->MlmeAux.SupRate,
			       &pAd->MlmeAux.SupRateLen);
		pAd->MlmeAux.ExtRateLen = pAd->PortCfg.ExtRateLen;
		memcpy(pAd->MlmeAux.ExtRate, pAd->PortCfg.ExtRate,
		       MAX_LEN_OF_SUPPORTED_RATES);
		...
		memset(&pAd->MlmeAux.APQosCapability, 0,
		...
		if (Bssidx == BSS_NOT_FOUND) {
			Bssidx =
			    BssTableSetEntry(pAd, &pAd->ScanTab,
					     pAd->MlmeAux.Bssid, Ssid, SsidLen,
					     pAd->MlmeAux.BssType,
					     pAd->MlmeAux.BeaconPeriod, &CfParm,
					     pAd->MlmeAux.AtimWin,
					     pAd->MlmeAux.CapabilityInfo,
					     pAd->MlmeAux.SupRate,
					     pAd->MlmeAux.SupRateLen,
					     ...
					     pAd->MlmeAux.ExtRateLen,
					     pAd->MlmeAux.Channel,
					     pAd->BbpRssiToDbmDelta - 30,
					     TimeStamp, 0, NULL, NULL, NULL, 0,
					     pVIE);
		}
#endif
...
//      DBGPRINT(RT_DEBUG_TRACE, "SYNC - PeerBeaconAtScanAction (SubType=%d, SsidLen=%d, Ssid=%s)\n", pFrame->Hdr.FC.SubType, SsidLen,Ssid);
		...
		if (pAd->PortCfg.bGetAPConfig) {
			...
			if (BackDoorProbeRspSanity
			    (pAd, Elem->Msg, Elem->MsgLen, CfgData)) {
		...
		Idx =
		    BssTableSetEntry(pAd, &pAd->ScanTab, Bssid, Ssid, SsidLen,
				     BssType, BeaconPeriod, &CfParm, AtimWin,
				     CapabilityInfo, SupRate, SupRateLen,
				     ExtRate, ExtRateLen, Channel, Rssi,
				     TimeStamp, CkipFlag, &EdcaParm,
...
//    UINT          FrameLen = 0;
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
		...
		if ((BssType == BSS_ADHOC) && (pAd->PortCfg.AdhocMode == 2)
		...
		if (memcmp(pAd->MlmeAux.Bssid, Bssid, ETH_ALEN) == 0) {
			DBGPRINT(RT_DEBUG_TRACE,
			...
			memcpy(pAd->MlmeAux.Bssid, Bssid, ETH_ALEN);
			pAd->MlmeAux.CapabilityInfo =
			...
			if (pAd->PortCfg.bWmmCapable) {
				memcpy(&pAd->MlmeAux.APEdcaParm, &EdcaParm,
				       sizeof(EDCA_PARM));
				memcpy(&pAd->MlmeAux.APQbssLoad, &QbssLoad,
				       sizeof(QBSS_LOAD_PARM));
				memcpy(&pAd->MlmeAux.APQosCapability,
				       &QosCapability,
				       sizeof(QOS_CAPABILITY_PARM));
			} else {
				memset(&pAd->MlmeAux.APEdcaParm, 0,
				       sizeof(EDCA_PARM));
				memset(&pAd->MlmeAux.APQbssLoad, 0,
				       sizeof(QBSS_LOAD_PARM));
				memset(&pAd->MlmeAux.APQosCapability, 0,
				       sizeof(QOS_CAPABILITY_PARM));
			}
...
VOID PeerBeacon(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
		...
		if (pAd->RxAnt.EvaluatePeriod == 0) {
			RealRssi =
			    (UCHAR) ConvertToRssi(pAd, Elem->Rssi, RSSI_NO_1);
		}

		is_my_bssid =
		    (memcmp(Bssid, pAd->PortCfg.Bssid, ETH_ALEN) ==
		     0) ? TRUE : FALSE;
		is_my_ssid =
		    SSID_EQUAL(Ssid, SsidLen, pAd->PortCfg.Ssid,
			       pAd->PortCfg.SsidLen) ? TRUE : FALSE;
		...
		if (Bssidx == BSS_NOT_FOUND) {
			...
			Bssidx =
			    BssTableSetEntry(pAd, &pAd->MlmeAux.SsidBssTab,
					     Bssid, Ssid, SsidLen, BssType,
					     BeaconPeriod, &CfParm, AtimWin,
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ...
				 Bssidx, RealRssi, Bssid[0], Bssid[1], Bssid[2],
				 Bssid[3], Bssid[4], Bssid[5]);
		}

		if ((pAd->PortCfg.bIEEE80211H == 1) && (NewChannel != 0)
		    && (Channel != NewChannel)) {
			...
			for (i = 0; i < 6; i++) {
				if (Bssid[i] > pAd->PortCfg.Bssid[i]) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ...
						 Bssid[3], Bssid[4], Bssid[5]);
					AsicDisableSync(pAd);
					memcpy(pAd->PortCfg.Bssid, Bssid,
					       ETH_ALEN);
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ...
			 Bssid[5], DtimCount, DtimPeriod, RealRssi);
		...
		if (is_my_bssid) {
			...
			if (ADHOC_ON(pAd) && (SupRateLen + ExtRateLen <= 4)) {
				...
				if (pAd->PortCfg.MaxTxRate > RATE_11) {
					...
					memcpy(pAd->ActiveCfg.SupRate, SupRate,
					       MAX_LEN_OF_SUPPORTED_RATES);
					pAd->ActiveCfg.SupRateLen = SupRateLen;
					memcpy(pAd->ActiveCfg.ExtRate, ExtRate,
					       MAX_LEN_OF_SUPPORTED_RATES);
			...
			if (INFRA_ON(pAd))	// && (pAd->PortCfg.PhyMode == PHY_11BG_MIXED))
			{
				...
				//      3. short preamble
				bUseShortSlot = pAd->PortCfg.UseShortSlotTime
				...
				// copy QOS related information
				memcpy(&pAd->PortCfg.APQbssLoad, &QbssLoad,
				       sizeof(QBSS_LOAD_PARM));
				memcpy(&pAd->PortCfg.APQosCapability,
				       &QosCapability,
				       sizeof(QOS_CAPABILITY_PARM));
...
//                       (RTMPFreeTXDRequest(pAd, QID_AC_VO, TX_RING_SIZE, &FreeNumber) != NDIS_STATUS_SUCCESS)       ||
					...
					DBGPRINT(RT_DEBUG_INFO,
						 "SYNC - PHY sleeps for %d TBTT, Dtim=%d/%d\n",
						 TbttNumToNextWakeUp, DtimCount,
						 DtimPeriod);
	...
	if (PeerProbeReqSanity
		...
		if ((SsidLen == 0)
			...
			CapabilityInfo =
			    CAP_GENERATE(0, 1, Privacy,
					 (pAd->PortCfg.TxPreamble ==
					  Rt802_11PreambleShort), 0);

			MakeOutgoingFrame(pOutBuffer, &FrameLen,
					  sizeof(HEADER_802_11), &ProbeRspHdr,
					  TIMESTAMP_LEN, &FakeTimestamp,
					  2, &pAd->PortCfg.BeaconPeriod,
					  2, &pAd->ActiveCfg.CapabilityInfo,
					  1, &SsidIe,
					  1, &pAd->PortCfg.SsidLen,
					  pAd->PortCfg.SsidLen,
					  pAd->PortCfg.Ssid, 1, &SupRateIe, 1,
					  &pAd->ActiveCfg.SupRateLen,
					  pAd->ActiveCfg.SupRateLen,
					  pAd->ActiveCfg.SupRate, 1, &DsIe, 1,
					  &DsLen, 1, &pAd->PortCfg.Channel, 1,
					  &IbssIe, 1, &IbssLen, 2,
					  &pAd->ActiveCfg.AtimWin, END_OF_ARGS);
			...
			if (pAd->ActiveCfg.ExtRateLen) {
				...
				MakeOutgoingFrame(pOutBuffer + FrameLen, &tmp,
						  ...
						  pAd->ActiveCfg.ExtRateLen,
						  &pAd->ActiveCfg.ExtRate,
						  END_OF_ARGS);
				FrameLen += tmp;
			}
...
static VOID InvalidStateWhenJoin(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 "AYNC - InvalidStateWhenJoin(state=%d). Reset SYNC machine\n",
		 pAd->Mlme.SyncMachine.CurrState);
	pAd->Mlme.SyncMachine.CurrState = SYNC_IDLE;
	Status = MLME_STATE_MACHINE_REJECT;
	MlmeEnqueue(pAd, MLME_CNTL_STATE_MACHINE, MT2_JOIN_CONF, 2, &Status);
...
static inline VOID InvalidStateWhenStart(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 "AYNC - InvalidStateWhenStart(state=%d). Reset SYNC machine\n",
		 pAd->Mlme.SyncMachine.CurrState);
	pAd->Mlme.SyncMachine.CurrState = SYNC_IDLE;
	Status = MLME_STATE_MACHINE_REJECT;
...
VOID EnqueuePsPoll(IN PRTMP_ADAPTER pAd)
{
	...
	MiniportMMRequest(pAd, (PUCHAR) & pAd->PsPollFrame,
			  sizeof(PSPOLL_FRAME));
}
...
//    DBGPRINT(RT_DEBUG_TRACE, "SYNC - driver sent BEACON (len=%d)...\n",pTxD->DataByteCnt);
	RTMP_IO_READ32(pAd, TXRX_CSR13, &Tsf.vv.HighPart);
	RTMP_IO_READ32(pAd, TXRX_CSR12, &Tsf.vv.LowPart);
	// TODO: not good if porting to big endian platform - TSF byte order ???
	memcpy(pBeacon + sizeof(HEADER_802_11), &Tsf, TIMESTAMP_LEN);
	MiniportMMRequest(pAd, pBeacon, pTxD->DataByteCnt);
...
VOID EnqueueProbeRequest(IN PRTMP_ADAPTER pAd)
{
	...
	if (pOutBuffer == NULL) {
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  ...
				  pAd->ActiveCfg.SupRateLen,
				  pAd->ActiveCfg.SupRate, END_OF_ARGS);

		if (pAd->PortCfg.bGetAPConfig) {
			UCHAR RalinkSpecificIEForGetCfg[6] =
			    { IE_VENDOR_SPECIFIC, 4, 0x00, 0x0c, 0x43, 0x80 };
			ULONG Tmp = 0;
			MakeOutgoingFrame(pOutBuffer + FrameLen, &Tmp,
					  6, RalinkSpecificIEForGetCfg,
					  END_OF_ARGS);
...
VOID BuildChannelList(IN PRTMP_ADAPTER pAd)
{
	...
	Channel = pAd->PortCfg.AdHocChannel;
	if (pAd->PortCfg.BssType == BSS_ADHOC) {
		if (ChannelSanity(pAd, Channel) == TRUE) {
			pAd->PortCfg.Channel = pAd->PortCfg.AdHocChannel;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ...
		 pAd->PortCfg.CountryRegion, pAd->PortCfg.CountryRegionForABand,
		 pAd->RfIcType, pAd->PortCfg.PhyMode, pAd->ChannelListNum);
	for (i = 0; i < index; i++) {
	...
	if (pAd->PortCfg.Channel <= 14) {
		...
		if (pAd->NicConfig2.field.ExternalLNAForG == 0) {
			...
			if (LnaAgc.field.Lna == 0x03)
				...
				return (LnaAgc.field.Agc * 2 - (64 + 14) +
					RssiOffset);
			else
				return -1;
		}
